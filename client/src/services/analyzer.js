...
/**
 * Full conversation analysis running entirely in the browser.
 */
...
function detectEmotions(text) {
  ...
  const emotionLexicon = {
    joy: ["happy","glad","love","amazing","awesome","great","wonderful","fantastic","excellent","beautiful","excited","yay","lol","haha","😂","😊","❤️","🥰","😍","celebrate","fun","enjoy"],
    anger: ["angry","mad","furious","hate","annoyed","frustrated","pissed","ugh","wtf","stupid","idiot","rage","disgusting","horrible","😡","🤬","damn","hell"],
    sadness: ["sad","depressed","cry","tears","miss","lonely","heartbroken","sorry","disappointed","unfortunately","😢","😭","💔","sigh","upset"],
    fear: ["scared","afraid","worried","anxious","nervous","panic","terrified","creepy","scary","fear","😰","😨","😱","help"],
    surprise: ["wow","omg","no way","really","seriously","what","unexpected","shocked","unbelievable","😮","😲","🤯","surprise","whoa"],
    disgust: ["gross","eww","disgusting","nasty","yuck","terrible","awful","🤢","🤮","cringe","icky"],
    sarcasm: ["sure","right","yeah right","totally","obviously","of course","wow how surprising","great job","genius","brilliant","🙄","whatever"],
    affection: ["love you","miss you","babe","baby","honey","sweetheart","cutie","😘","💕","💗","xo","muah","hugs","kisses"],
  };
...
function detectTone(text, sentimentResult) {
  ...
  if (/!{2,}|asap|urgent|hurry|now|immediately|quick/i.test(text)) tones.push("urgent");
  if (/haha|lol|lmao|😂|🤣|jk|kidding|joking/i.test(text)) tones.push("playful");
  if (/k\.|fine\.|whatever|sure\.|okay\.|👍$/i.test(text)) tones.push("passive-aggressive");
  if (/😏|😘|😍|🥰|💋|👀|cutie|hottie|babe|gorgeous/i.test(text)) tones.push("flirty");
  if (sentimentResult.label === "neutral" && text.length < 50) tones.push("dry");
...
function generateSummary(sentiment, emotions, tone, themes) {
  ...
  if (emotions.dominant !== "neutral") parts.push(`The dominant emotion is ${emotions.dominant}.`);
